...
public class BoardTestSuite {
    ...
    @Test
    public void testAddTaskListAverageTasksInProgress(){
        //Given
        Board project = prepereTestData();
        ...
        List<TaskList> inProgressTask = new ArrayList<>();
        inProgressTask.add(new TaskList("In progress"));
       double averageTimeForTask = project.getTaskLists().stream()
               .filter(inProgressTask::contains)
               .flatMap(tl->tl.getTasks().stream())
               .mapToLong(d->d.getCreated().until(LocalDate.now(),DAYS))
               .average().getAsDouble();

       //Then
        Assert.assertEquals(10.00, averageTimeForTask, 0.01);
    }
}
